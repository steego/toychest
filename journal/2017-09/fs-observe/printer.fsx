...
let rec print (level:int) = 
    let rec print (level:int) (o:obj) : Html.Tag = 
        ...
        // | GenericList(t, getters, list) when t.ElementType.IsPrimitive ->
        //     if level < 1 then Html.Text("List...") else printList (level - 1) list
    ...
    and printGenericList (level:int) (t:TypeInfo) (getters:MemberGetter list) (list:IEnumerable) =
        let list = seq { for o in list -> o }
        if t.ElementType.IsPrimitive then
            table
                [("class", "table table-bordered table-striped")]
                [
                    // thead [] [ tr [] [ th [] [Html.Text("Primitive: " + t.FullName)] ]]
                    tbody [] [ for item in list.Take(maxTake) do
                                 yield tr [] [ td [] [ item |> print level ] ]  ]
                ]
        else
            table 
                [("class", "table table-bordered table-striped")]
                [ 
                    thead [] [ 
                        // yield tr [] [ th [] [Html.Text("Not-Primitive: " + t.FullName)] ]
                        for g in getters -> th [] [ Html.Text(g.Name) ] 
                    ]
                    tbody [] [ for item in list.Take(maxTake) do
